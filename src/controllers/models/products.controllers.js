...
export const Insert_Products = async (params) =>{
    ...
    try {
        await pool.query(
            ...
            [data.code, data.category, data.brand, data.product, data.price, data.inventory, data.description ]
        ); 
...
const Prepare_Products_Sql = (params) => {
    try {
        ...
        const {code, category, brand, product, price, oldprice, inventory, productnew, discount,  description, status,   page, perPage } = params;
        const conditions = [];
            if (code) {
...
export const Update_Products= async (data) => {  
    ...
    const {category, brand, product, price, oldprice, inventory, productnew, discount,  description, status} = body;
    const {id} = params;
    let sql = null;
    try {
